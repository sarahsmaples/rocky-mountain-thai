...
const zoomImgs = document.querySelectorAll('[data-scroll-zoom]');
if (zoomImgs.length) {
  const updateZoom = () => {
    ...
    zoomImgs.forEach(img => {
      ...
      // progress: 0 when center is at bottom of viewport, 1 when at top
      const progress = 1 - (center / vh);
      const scale = 0.9 + Math.max(0, Math.min(progress, 1)) * 0.1;
